...
class BearFlagBreakdownStrategy:
    def __init__(self, symbol, timeframe=mt5.TIMEFRAME_M15, lot_size=0.1, impulse_lookback=20, flag_lookback=12, atr_period=14, atr_sl_mult=1.4, rr_multiple=2.8, max_trades_per_day=3, magic=510012, timezone='Asia/Kolkata'):
        self.symbol = symbol
    ...
    def _bear_flag(self, df):
        ...
        impulse_drop = imp['open'].iloc[0] - imp['close'].iloc[-1]
        if impulse_drop <= 2.0 * imp['atr'].mean():
            return None
        if flag['low'].min() <= imp['low'].iloc[-1] * 0.99:
            return None
        pivot = float(flag['low'].min())
        return pivot
    ...
    def get_signal(self, df):
        ...
        if prev['close'] >= pivot and last['close'] < pivot and last['close'] < last['open']:
            return Signal('SELL', round(pivot, 5), round(last['atr'], 5), last['time'])
        return None
    ...
    def place_order(self, signal):
        ...
        request = {'action': mt5.TRADE_ACTION_DEAL, 'symbol': self.symbol, 'volume': self.lot_size, 'type': mt5.ORDER_TYPE_SELL, 'price': entry, 'sl': round(sl, 5), 'tp': round(tp, 5), 'deviation': 10, 'magic': self.magic, 'comment': 'BEAR_FLAG_BREAK', 'type_time': mt5.ORDER_TIME_GTC, 'type_filling': mt5.ORDER_FILLING_IOC}
