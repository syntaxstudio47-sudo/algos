...
class HighTightFlagStrategy:
    def __init__(self, symbol, timeframe=mt5.TIMEFRAME_H1, lot_size=0.1, pole_lookback=30, flag_lookback=15, atr_period=14, atr_sl_mult=1.5, rr_multiple=3.0, max_trades_per_day=2, magic=510014, timezone='Asia/Kolkata'):
        self.symbol = symbol
    ...
    def get_candles(self, count=260):
    ...
    def _detect(self, df):
        w = df.tail(self.pole_lookback + self.flag_lookback)
        if len(w) < self.pole_lookback + self.flag_lookback:
            return None
        pole = w.iloc[:self.pole_lookback]
        flag = w.iloc[self.pole_lookback:]
        pole_gain = (pole['close'].iloc[-1] - pole['open'].iloc[0]) / pole['open'].iloc[0]
        if pole_gain < 0.08:
            return None
        if flag['low'].min() < pole['close'].iloc[-1] * 0.9:
            return None
        return float(flag['high'].max())

    def get_signal(self, df):
        if len(df) < self.pole_lookback + self.flag_lookback + 5:
            return None
        df = df.copy()
        df['atr'] = self._atr(df)
        pivot = self._detect(df)
        if pivot is None:
            return None
        last = df.iloc[-1]
        prev = df.iloc[-2]
    ...
    def place_order(self, signal):
        ...
        request = {'action': mt5.TRADE_ACTION_DEAL, 'symbol': self.symbol, 'volume': self.lot_size, 'type': mt5.ORDER_TYPE_BUY, 'price': entry, 'sl': round(sl, 5), 'tp': round(tp, 5), 'deviation': 15, 'magic': self.magic, 'comment': 'HTF_BREAK', 'type_time': mt5.ORDER_TIME_GTC, 'type_filling': mt5.ORDER_FILLING_IOC}
