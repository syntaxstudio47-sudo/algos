...
class VolumeExpansionBreakoutStrategy:
    def __init__(self, symbol, timeframe=mt5.TIMEFRAME_M15, lot_size=0.1, breakout_lookback=20, volume_lookback=20, volume_mult=1.5, atr_period=14, atr_sl_mult=1.4, rr_multiple=2.6, max_trades_per_day=3, magic=510003, timezone='Asia/Kolkata'):
        self.symbol = symbol
    ...
    def get_signal(self, df):
        if len(df) < max(self.breakout_lookback, self.volume_lookback) + 5:
            return None
        df = df.copy()
        df['atr'] = self._atr(df)
        last = df.iloc[-1]
        ...
        vol_mean = df['tick_volume'].shift(1).rolling(self.volume_lookback).mean().iloc[-1]
        vol_ok = last['tick_volume'] >= vol_mean * self.volume_mult
        if vol_ok and prev['close'] <= breakout_high and last['close'] > breakout_high:
            return Signal('BUY', round(breakout_high, 5), round(last['atr'], 5), last['time'])
        if vol_ok and prev['close'] >= breakout_low and last['close'] < breakout_low:
    ...
    def place_order(self, signal):
        ...
        request = {'action': mt5.TRADE_ACTION_DEAL, 'symbol': self.symbol, 'volume': self.lot_size, 'type': order_type, 'price': entry, 'sl': round(sl, 5), 'tp': round(tp, 5), 'deviation': 10, 'magic': self.magic, 'comment': f'VOL_EXP_{signal.signal}', 'type_time': mt5.ORDER_TIME_GTC, 'type_filling': mt5.ORDER_FILLING_IOC}
