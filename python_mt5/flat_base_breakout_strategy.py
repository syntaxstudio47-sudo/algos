...
class FlatBaseBreakoutStrategy:
    def __init__(self, symbol, timeframe=mt5.TIMEFRAME_M15, lot_size=0.1, lookback=30, max_base_depth_pct=0.08, atr_period=14, atr_sl_mult=1.4, rr_multiple=2.6, max_trades_per_day=3, magic=510007, timezone='Asia/Kolkata'):
        self.symbol = symbol
    ...
    def _detect_base(self, df):
        base = df.tail(self.lookback)
        top = float(base['high'].max())
        bottom = float(base['low'].min())
        if top <= 0:
            return None
        depth_pct = (top - bottom) / top
        if depth_pct <= self.max_base_depth_pct:
            return top, bottom
        return None
    ...
    def get_signal(self, df):
        ...
        df = df.copy()
        df['atr'] = self._atr(df)
        setup = self._detect_base(df.iloc[:-1])
        if setup is None:
            return None
        top, bottom = setup
        last = df.iloc[-1]
        ...
        if prev['close'] <= top and last['close'] > top:
            return Signal('BUY', round(top, 5), round(last['atr'], 5), last['time'])
        return None
    ...
    def place_order(self, signal):
        ...
        request = {'action': mt5.TRADE_ACTION_DEAL, 'symbol': self.symbol, 'volume': self.lot_size, 'type': mt5.ORDER_TYPE_BUY, 'price': entry, 'sl': round(sl, 5), 'tp': round(tp, 5), 'deviation': 10, 'magic': self.magic, 'comment': 'FLAT_BASE_BREAK', 'type_time': mt5.ORDER_TIME_GTC, 'type_filling': mt5.ORDER_FILLING_IOC}
