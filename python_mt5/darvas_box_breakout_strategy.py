...
class DarvasBoxBreakoutStrategy:
    def __init__(self, symbol, timeframe=mt5.TIMEFRAME_M15, lot_size=0.1, box_lookback=20, volume_lookback=20, volume_mult=1.3, atr_period=14, atr_sl_mult=1.5, rr_multiple=2.8, max_trades_per_day=3, magic=510006, timezone='Asia/Kolkata'):
        self.symbol = symbol
    ...
    def get_signal(self, df):
        if len(df) < max(self.box_lookback, self.volume_lookback) + 5:
            return None
        df = df.copy()
        df['atr'] = self._atr(df)
        last = df.iloc[-1]
        ...
        if pd.isna(last['atr']):
            return None
        box_high = df['high'].shift(1).rolling(self.box_lookback).max().iloc[-1]
        box_low = df['low'].shift(1).rolling(self.box_lookback).min().iloc[-1]
        volume_avg = df['tick_volume'].shift(1).rolling(self.volume_lookback).mean().iloc[-1]
        vol_ok = last['tick_volume'] >= volume_avg * self.volume_mult
        if vol_ok and prev['close'] <= box_high and last['close'] > box_high:
            return Signal('BUY', round(box_high, 5), round(last['atr'], 5), last['time'])
        if vol_ok and prev['close'] >= box_low and last['close'] < box_low:
            return Signal('SELL', round(box_low, 5), round(last['atr'], 5), last['time'])
        return None
    ...
    def place_order(self, signal):
        ...
        request = {'action': mt5.TRADE_ACTION_DEAL, 'symbol': self.symbol, 'volume': self.lot_size, 'type': order_type, 'price': entry, 'sl': round(sl, 5), 'tp': round(tp, 5), 'deviation': 10, 'magic': self.magic, 'comment': f'DARVAS_{signal.signal}', 'type_time': mt5.ORDER_TIME_GTC, 'type_filling': mt5.ORDER_FILLING_IOC}
