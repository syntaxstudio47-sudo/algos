...
class AscendingTriangleBreakoutStrategy:
    def __init__(self, symbol, timeframe=mt5.TIMEFRAME_M15, lot_size=0.1, lookback=40, atr_period=14, atr_sl_mult=1.5, rr_multiple=2.5, max_trades_per_day=3, magic=510001, timezone='Asia/Kolkata'):
        self.symbol = symbol
    ...
    def _detect_triangle(self, df):
        ...
        upper = highs.max()
        lower = lows.min()
        if high_slope < 0.0 and low_slope > -0.02 and low_slope >= -abs(high_slope) * 0.25:
    ...
    def get_signal(self, df):
        ...
        if pd.isna(last['atr']):
            return None
        if prev['close'] <= upper and last['close'] > upper and last['close'] > last['open']:
            return Signal('BUY', round(upper, 5), round(last['atr'], 5), last['time'])
        return None
    ...
    def place_order(self, signal):
        ...
        request = {'action': mt5.TRADE_ACTION_DEAL, 'symbol': self.symbol, 'volume': self.lot_size, 'type': mt5.ORDER_TYPE_BUY, 'price': entry, 'sl': round(sl, 5), 'tp': round(tp, 5), 'deviation': 10, 'magic': self.magic, 'comment': 'ASC_TRI_BREAK', 'type_time': mt5.ORDER_TIME_GTC, 'type_filling': mt5.ORDER_FILLING_IOC}
