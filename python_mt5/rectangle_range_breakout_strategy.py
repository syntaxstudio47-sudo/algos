...
class RectangleRangeBreakoutStrategy:
    def __init__(self, symbol, timeframe=mt5.TIMEFRAME_M15, lot_size=0.1, range_lookback=30, atr_period=14, atr_sl_mult=1.5, rr_multiple=2.5, max_trades_per_day=3, magic=510004, timezone='Asia/Kolkata'):
        self.symbol = symbol
    ...
    def get_signal(self, df):
        ...
        if pd.isna(last['atr']):
            return None
        zone = df.tail(self.range_lookback)
        resistance = zone['high'].max()
        support = zone['low'].min()
        width = resistance - support
        if width <= last['atr'] * 1.0:
            return None
        if prev['close'] <= resistance and last['close'] > resistance:
            return Signal('BUY', round(resistance, 5), round(last['atr'], 5), last['time'])
    ...
    def place_order(self, signal):
        ...
        request = {'action': mt5.TRADE_ACTION_DEAL, 'symbol': self.symbol, 'volume': self.lot_size, 'type': order_type, 'price': entry, 'sl': round(sl, 5), 'tp': round(tp, 5), 'deviation': 10, 'magic': self.magic, 'comment': f'RECT_BREAK_{signal.signal}', 'type_time': mt5.ORDER_TIME_GTC, 'type_filling': mt5.ORDER_FILLING_IOC}
