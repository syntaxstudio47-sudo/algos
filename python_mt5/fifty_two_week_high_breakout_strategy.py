...
class FiftyTwoWeekHighBreakoutStrategy:
    def __init__(self, symbol, timeframe=mt5.TIMEFRAME_D1, lot_size=0.1, lookback=252, atr_period=14, atr_sl_mult=2.0, rr_multiple=3.0, max_trades_per_day=1, magic=510013, timezone='Asia/Kolkata'):
        self.symbol = symbol
    ...
    def get_candles(self, count=400):
    ...
    def get_signal(self, df):
        ...
        last = df.iloc[-1]
        prev = df.iloc[-2]
        fifty_two_high = df['high'].shift(1).rolling(self.lookback).max().iloc[-1]
        if pd.isna(last['atr']):
            return None
        if prev['close'] <= fifty_two_high and last['close'] > fifty_two_high:
            return Signal('BUY', round(fifty_two_high, 5), round(last['atr'], 5), last['time'])
        return None
    ...
    def place_order(self, signal):
        ...
        request = {'action': mt5.TRADE_ACTION_DEAL, 'symbol': self.symbol, 'volume': self.lot_size, 'type': mt5.ORDER_TYPE_BUY, 'price': entry, 'sl': round(sl, 5), 'tp': round(tp, 5), 'deviation': 20, 'magic': self.magic, 'comment': '52W_HIGH_BREAK', 'type_time': mt5.ORDER_TIME_GTC, 'type_filling': mt5.ORDER_FILLING_IOC}
