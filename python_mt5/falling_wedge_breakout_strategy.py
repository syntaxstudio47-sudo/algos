...
class FallingWedgeBreakoutStrategy:
    def __init__(self, symbol, timeframe=mt5.TIMEFRAME_M15, lot_size=0.1, lookback=35, atr_period=14, atr_sl_mult=1.4, rr_multiple=2.6, max_trades_per_day=3, magic=510015, timezone='Asia/Kolkata'):
        self.symbol = symbol
    ...
    def _detect(self, df):
        w = df.tail(self.lookback)
        x = np.arange(len(w))
        hs = np.polyfit(x, w['high'].values, 1)[0]
        ls = np.polyfit(x, w['low'].values, 1)[0]
    ...
    def get_signal(self, df):
        ...
        if prev['close'] <= level and last['close'] > level:
            return Signal('BUY', round(level, 5), round(last['atr'], 5), last['time'])
        return None
    ...
    def place_order(self, signal):
        ...
        request = {'action': mt5.TRADE_ACTION_DEAL, 'symbol': self.symbol, 'volume': self.lot_size, 'type': mt5.ORDER_TYPE_BUY, 'price': entry, 'sl': round(sl, 5), 'tp': round(tp, 5), 'deviation': 10, 'magic': self.magic, 'comment': 'FALL_WEDGE_BREAK', 'type_time': mt5.ORDER_TIME_GTC, 'type_filling': mt5.ORDER_FILLING_IOC}
