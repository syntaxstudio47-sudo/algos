...
input double ExpansionMult=1.4;
input double ATRSLMult=1.3;
input double RR=2.5;
input int MaxTradesPerDay=2;
input ulong Magic=620005;
...
void OnTick(){ if(!NewBar()) return; ResetDay(); if(tradesToday>=MaxTradesPerDay || HasPosition()) return; double atr=ATR(); if(atr<=0) return; double hi=-DBL_MAX, lo=DBL_MAX; for(int i=2;i<Lookback+2;i++){ hi=MathMax(hi,iHigh(_Symbol,_Period,i)); lo=MathMin(lo,iLow(_Symbol,_Period,i)); }
 double c1=iClose(_Symbol,_Period,1), c2=iClose(_Symbol,_Period,2), h1=iHigh(_Symbol,_Period,1), l1=iLow(_Symbol,_Period,1); double range=h1-l1; if(range<atr*ExpansionMult) return; trade.SetExpertMagicNumber(Magic);
 if(c2<=hi && c1>hi){ double ask=SymbolInfoDouble(_Symbol,SYMBOL_ASK); double sl=ask-atr*ATRSLMult; double tp=ask+atr*ATRSLMult*RR; if(trade.Buy(Lots,_Symbol,ask,sl,tp,"ATRExpansionBuy")) tradesToday++; }
 else if(c2>=lo && c1<lo){ double bid=SymbolInfoDouble(_Symbol,SYMBOL_BID); double sl=bid+atr*ATRSLMult; double tp=bid-atr*ATRSLMult*RR; if(trade.Sell(Lots,_Symbol,bid,sl,tp,"ATRExpansionSell")) tradesToday++; } }
