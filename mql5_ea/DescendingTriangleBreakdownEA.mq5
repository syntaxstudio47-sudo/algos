...
void OnTick(){ if(!NewBar()) return; ResetDay(); if(tradesToday>=MaxTradesPerDay || HasPosition()) return; double atr=ATR(); if(atr<=0) return; double highs[], lows[]; ArrayResize(highs,Lookback); ArrayResize(lows,Lookback); double support=DBL_MAX;
 for(int i=0;i<Lookback;i++){ highs[i]=iHigh(_Symbol,_Period,Lookback-i); lows[i]=iLow(_Symbol,_Period,Lookback-i); support=MathMin(support,lows[i]); }
 double hs=Slope(highs), ls=Slope(lows), c1=iClose(_Symbol,_Period,1), c2=iClose(_Symbol,_Period,2); if(!(hs<0 && MathAbs(ls)<MathAbs(hs)*0.3)) return; trade.SetExpertMagicNumber(Magic);
 if(c2>=support && c1<support){ double bid=SymbolInfoDouble(_Symbol,SYMBOL_BID); double sl=bid+atr*ATRSLMult; double tp=bid-atr*ATRSLMult*RR; if(trade.Sell(Lots,_Symbol,bid,sl,tp,"DescendingTriangleBreakdown")) tradesToday++; } }
