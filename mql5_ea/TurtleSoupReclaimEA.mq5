...
void OnTick(){ if(!NewBar()) return; ResetDay(); if(tradesToday>=MaxTradesPerDay || HasPosition()) return; double atr=ATR(); if(atr<=0) return; double hi=-DBL_MAX, lo=DBL_MAX; for(int i=2;i<Lookback+2;i++){ hi=MathMax(hi,iHigh(_Symbol,_Period,i)); lo=MathMin(lo,iLow(_Symbol,_Period,i)); }
 double h1=iHigh(_Symbol,_Period,1), l1=iLow(_Symbol,_Period,1), c1=iClose(_Symbol,_Period,1); trade.SetExpertMagicNumber(Magic);
 if(h1>hi && c1<hi){ double bid=SymbolInfoDouble(_Symbol,SYMBOL_BID); double sl=bid+atr*ATRSLMult; double tp=bid-atr*ATRSLMult*RR; if(trade.Sell(Lots,_Symbol,bid,sl,tp,"TurtleSoupSell")) tradesToday++; }
 else if(l1<lo && c1>lo){ double ask=SymbolInfoDouble(_Symbol,SYMBOL_ASK); double sl=ask-atr*ATRSLMult; double tp=ask+atr*ATRSLMult*RR; if(trade.Buy(Lots,_Symbol,ask,sl,tp,"TurtleSoupBuy")) tradesToday++; } }
